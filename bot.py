...
def gifts_keyboard(lang="uz"):
    """Display gifts in 2-column format with emoji, name and price"""
    buttons = []
    ...
    for i in range(0, len(gift_items), 2):
        ...
        row.append(InlineKeyboardButton(
            text=f"{emoji1} {name1} - {price1:,}",
            callback_data=f"gift_{key1}"
        ))
        ...
        if i + 1 < len(gift_items):
            ...
            row.append(InlineKeyboardButton(
                text=f"{emoji2} {name2} - {price2:,}",
                callback_data=f"gift_{key2}"
            ))
...
def admin_order_keyboard(order_id: int, user_id: int, order_type: str = "stars"):
    """Admin buttons: Start execution / Confirm / Cancel"""
    return InlineKeyboardMarkup(inline_keyboard=[
        [
...
async def recipient_self(callback: types.CallbackQuery, state: FSMContext):
    """FIXED: Handle buy for myself properly"""
    lang = get_user_lang(callback.from_user.id)
...
async def process_stars_order(msg, data: dict, lang: str, state: FSMContext, user_id: int, from_user: types.User):
    """Create stars order and show confirmation"""
    ...
    order_text = (
        ...
        f"🎯 Получатель: <b>{order['recipient']}</b>\n\n"
        f"📅 Дата: <b>{_dt.strftime('%Y-%m-%d')}</b>\n"
        f"⏰ Время: <b>{_dt.strftime('%H:%M:%S')}</b>"
    )
    ...
    try:
        await bot.send_message(
            CHAT_ID,
            order_text,
            reply_markup=admin_order_keyboard(order_id, user_id, "stars"),
            parse_mode="HTML"
        )
    ...
    # Show confirmation
    confirmation_text = t(lang, "order_confirmation",
        service="Telegram Stars ⭐️",
        amount=f"{amount} ⭐️",
        recipient=recipient,
        price=price,
        card=CARD_NUMBER
...
async def receive_stars_payment_proof(message: types.Message, state: FSMContext):
    ...
    receipt_msg = t(lang, "receipt_accepted")
    await message.answer(receipt_msg)

    # Yangi zakaz xabari
    new_order_text = (
        "🆕 <b>Yangi zakaz!</b>\n\n"
        "Siz yangi buyurtma berdingiz.\n"
        "Admin tekshirishi tugagach, xizmat yetkaziladi. ✅"
        if lang == "uz" else
        "🆕 <b>Новый заказ!</b>\n\n"
...
async def process_premium_order(msg, data: dict, lang: str, state: FSMContext, user_id: int, from_user: types.User):
    """Create premium order and show confirmation"""
    ...
    order_text = (
        ...
        f"📦 Количество: <b>{order['months']} oy</b>\n"
        f"💰 Сумма: <b>{order['price']:,} UZS</b>\n"
        f"🎯 Получатель: <b>{order['recipient']}</b>\n\n"
        f"📅 Дата: <b>{_dt2.strftime('%Y-%m-%d')}</b>\n"
        f"⏰ Время: <b>{_dt2.strftime('%H:%M:%S')}</b>"
    )
    ...
    try:
        await bot.send_message(
            CHAT_ID,
            order_text,
            reply_markup=admin_order_keyboard(order_id, user_id, "premium"),
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Error sending to chat: {e}")
...
async def receive_premium_payment_proof(message: types.Message, state: FSMContext):
    ...
    chek_caption = (
        f"📸 <b>Чек для заказа #{order_id}</b> (💎 Premium)\n"
        f"👤 Пользователь: @{message.from_user.username or message.from_user.full_name}\n"
        f"🆔 ID: <code>{message.from_user.id}</code>"
    )
...
async def select_gift(callback: types.CallbackQuery, state: FSMContext):
    """FIXED: Properly handle gift selection"""
    lang = get_user_lang(callback.from_user.id)
    ...
    price_text = (
        f"💝 <b>{gift_display}</b>\n💴 Narx: <b>{price:,} UZS</b>\n\n"
        f"❗️ Qabul qiluvchining @username kiriting, yoki o'zingiz uchun tugmani bosing:"
        if lang == "uz"
        else
        f"💝 <b>{gift_display}</b>\n💴 Цена: <b>{price:,} UZS</b>\n\n"
        f"❗️ Введите @username получателя, или нажмите кнопку ниже если для себя:"
    )
...
async def gift_recipient_self(callback: types.CallbackQuery, state: FSMContext):
    """FIXED: Handle buy for myself for gifts"""
    lang = get_user_lang(callback.from_user.id)
...
async def enter_gift_username(message: types.Message, state: FSMContext):
    """FIXED: Handle gift username entry"""
    lang = get_user_lang(message.from_user.id)
    username = message.text.strip()
    if not username.startswith("@"):
        username = "@" + username
...
async def process_gift_order(msg, data: dict, lang: str, state: FSMContext, user_id: int, from_user: types.User):
    """FIXED: Create gift order properly"""
    ...
    order_text = (
        ...
        f"🆔 User ID: <code>{order['user_id']}</code>\n\n"
        f"🎁 Услуга: <b>Sovga</b>\n"
        f"📦 Количество: <b>{order['gift_emoji']} {order['gift_name']}</b>\n"
        f"💰 Сумма: <b>{order['price']:,} UZS</b>\n"
        f"🎯 Получатель: <b>{order['recipient']}</b>\n\n"
        f"📅 Дата: <b>{_dt3.strftime('%Y-%m-%d')}</b>\n"
        f"⏰ Время: <b>{_dt3.strftime('%H:%M:%S')}</b>"
    )
    ...
    try:
        await bot.send_message(
            CHAT_ID,
            order_text,
            reply_markup=admin_order_keyboard(order_id, user_id, "gift"),
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Error sending to chat: {e}")
    ...
    # Show confirmation
    confirmation_text = t(lang, "gift_order_confirmation",
        gift_name=gift_name,
        recipient=recipient,
        price=price,
        card=CARD_NUMBER
...
async def receive_gift_payment_proof(message: types.Message, state: FSMContext):
    """FIXED: Handle gift payment receipt"""
    lang = get_user_lang(message.from_user.id)
    data = await state.get_data()
    order_id = data.get("order_id", "?")

    chek_caption = (
        f"📸 <b>Чек для заказа #{order_id}</b> (🎁 Подарок)\n"
        f"👤 Пользователь: @{message.from_user.username or message.from_user.full_name}\n"
        f"🆔 ID: <code>{message.from_user.id}</code>"
    )
    ...
    try:
        ...
        await bot.send_message(
            CHAT_ID,
            chek_caption,
            reply_markup=admin_order_keyboard(order_id, message.from_user.id, "gift"),
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Error sending to chat: {e}")
...
async def admin_confirm(callback: types.CallbackQuery):
    ...
    # Otziv kanalga avtomatik xabar yuborish
    if REVIEW_CHANNEL_ID != 0:
        ...
        if order:
            ...
            review_text = (
                f"✅ <b>Yangi muvaffaqiyatli xarid!</b>\n\n"
                f"📦 Xizmat: <b>{service_text}</b>\n"
                f"👤 Mijoz: @{username}\n"
                f"📥 Qabul qiluvchi: <b>{recipient}</b>\n"
                f"💰 Summa: <b>{price:,} UZS</b>\n\n"
                f"⭐ <b>Star Shop</b> dan mamnun mijoz!\n"
                f"👉 @starshopuzb_bot"
            )
            try:
...
async def admin_cancel(callback: types.CallbackQuery):
    ...
    await bot.send_message(
        user_id,
        t(user_lang, "rejected_msg", oid=order_id, admin=ADMIN_USERNAME.lstrip("@")),
        parse_mode="HTML"
    )
...
async def withdraw_bonus(callback: types.CallbackQuery):
    ...
    await callback.message.answer(
        t(lang, "select_withdrawal"),
        reply_markup=withdrawal_keyboard(lang),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def process_withdraw(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        t(lang, "bonus_withdraw_req", stars=bonus_stars, amount=amount, admin=ADMIN_USERNAME.lstrip("@")),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def admin_panel(message: types.Message):
    ...
    await message.answer(
        ...
        f"💰 Total Revenue: <b>{total_revenue:,} UZS</b>",
        parse_mode="HTML"
    )
